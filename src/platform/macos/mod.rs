...
/// macOS display backend. `detect_platform` returns this on macOS. The actual
/// window + wgpu-surface render loop is still a stub — see [`MacOSDisplay`] and
/// the module-level notes above.
...
impl DisplayPlatform for MacOSPlatform {
    fn spawn_wallpaper(
        ...
        // ponytail: stub — wire to a winit window + wgpu surface + render loop
        // (mirror wayland/mod.rs `spawn_wayland_wallpaper`) once winit/objc2
        // are added to Cargo.toml.
        anyhow::bail!(
            "macOS wallpaper rendering not yet implemented — \
             MacOSPlatform::spawn_wallpaper is a stub"
        )
    }
}
